...
//	N_mm = mMesh.size_faces();
//	N_f = mFrac.n_frac.size();
//	N_fm = mFrac.n_frac.size();
//	N_ff = mFrac.inter_nn.coor.size();
//	fCells.resize(N_f);
//	ffFaces.resize(N_ff);
//	mfFaces.resize(N_fm);
//	N_c = N_f + N_m;
			     ...
			     const std::vector<double> &vKZ  )
{
	...
	for (std::size_t i = 0; i < nw; i++)
	{
		...
		//std::cout << "WI = " << mWells[i].WI << std::endl;
	}
